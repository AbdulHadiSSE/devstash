    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3">
        {pinnedItems.map((item) => {
          ...
          return (
            <Card key={item.id} className="w-full bg-zinc-950 border border-zinc-800 hover:bg-zinc-900/50 transition-colors cursor-pointer border-l-2 border-l-blue-500 rounded-xl overflow-hidden">
              <div className="flex items-start justify-between p-5">
                {/* Left Section */}
                <div className="flex items-start gap-4 flex-1 min-w-0">
                  {/* Icon */}
                  <div className="p-2.5 rounded-lg shrink-0 mt-0.5" style={{ backgroundColor: `${color}15`, color }}>
                  ...
                  <div className="flex flex-col gap-1 min-w-0 w-full">
                    <div className="flex items-center gap-2">
                      ...
                      <Pin className="h-3 w-3 text-zinc-500 fill-zinc-500 -rotate-45 shrink-0" />
                    ...
                    <div className="flex gap-2 mt-2">
                      {item.tags?.slice(0, 3).map(tag => (
                        <Badge key={tag} variant="secondary" className="bg-zinc-800/60 text-zinc-300 hover:bg-zinc-700 text-[10px] h-5 px-2 font-medium border border-zinc-700/50">
                          {tag}
                        </Badge>
                      ))}
                    </div>
