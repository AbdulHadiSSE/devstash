...
- **`src/proxy.ts` is the route-protection middleware** — in Next.js 16 the `middleware.ts` file is renamed to `proxy.ts` and the convention export is `proxy` (not `middleware`). Don't look for a `middleware.ts`; there isn't one. It wraps NextAuth's `auth()` and redirects unauthenticated requests to `/sign-in`, with `matcher: ["/dashboard/:path*", "/profile/:path*"]`.
